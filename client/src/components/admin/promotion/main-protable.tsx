...
const PromotionAdminMain: React.FC = () => {
  ...
  const [dataDetailModal, setDataDetailModal] = useState<PromotionDTO | null>(
    null
  );
  ...
  const columns: ProColumns<PromotionDTO>[] = [
    ...
    },
    {
      ...
      render: (_, record) => (
        <Tag
          color={record.discountType === "DISCOUNT_AMOUNT" ? "blue" : "orange"}
        >
    ...
    <>
      ...
      <div style={{ padding: "0 20px 20px 20px" }}>
        <ProTable<PromotionDTO>
          actionRef={actionRef}
          columns={columns}
          // Tham số thứ 3 (filter) không dùng tới nên đặt tên để ProTable đúng type nhưng tránh warning
          request={async (params, sort) => {
            ...
            try {
              ...
              // Log để debug
              console.log(
                `🔍 [Promotion Table #${currentRequestId}] Request params:`,
                {
                  params,
                  sort,
              ...
              console.log(
                `📤 [Promotion Table #${currentRequestId}] Sending request:`,
                queryParams
              );
              const startTime = Date.now();

              const response =
                await promotionService.getAllPromotions(queryParams);

              // Kiểm tra nếu request này đã bị override bởi request mới hơn
              if (currentRequestId !== requestIdRef.current) {
                console.log(
                  `⚠️ [Promotion Table #${currentRequestId}] Request bị hủy, có request mới hơn`
                );
                return {
                  data: [],
                  success: false,
              ...
              console.log(
                `📥 [Promotion Table #${currentRequestId}] Response:`,
                {
